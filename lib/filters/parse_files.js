...
function parseFiles (result) {
  ...
  forEach(files, (file, filename) => {
    ...
    if (name.match(/^templates/)) {
      ...
      var newFiles = parseFile(raw, { source: name, filename: newName })

      // Transform languages?
      transformLang(newFiles[newName], result.meta)

      // Add additional metadata
      Object.assign(newFiles[newName], {
        layout: 'styleguide',
        type: 'text/html',
        toc: result.toc // see addToc()
      })
